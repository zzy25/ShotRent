...
CREATE TABLE `users`(
  ...
  permission TINYINT(1) NOT NULL DEFAULT 0 COMMENT '0: 普通用户， 1：房子业主 2：系统管理员',
...
CREATE TABLE `houses`(
  ...
  houseType TINYINT(1) NOT NULL DEFAULT 0 COMMENT '0：平层户型， 1：跃层户型， 2：错层户型， 3：复式户型',
  ...
  online TINYINT(1) NOT NULL DEFAULT 1 COMMENT '房屋是否发布，0： 不发布， 1：发布',
  checked TINYINT(1) NOT NULL DEFAULT 0 COMMENT '房屋是否检查通过，0： 不通过， 1：通过',
  rented TINYINT(1) NOT NULL DEFAULT 0 COMMENT '房屋是否已经被租住， 0：未被租住， 1：已被租住',
...
CREATE TABLE `orders`(
  ...
  terminate TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否提前终止，0：不终止 1：终止'
